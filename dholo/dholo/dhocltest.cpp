...
namespace dholo
{
	namespace test
	{
		...
		void DHOCLTest::LoadImg(const ogl::DHOGLRender & scene)
		{
			...
			m_Mem[0] = clCreateFromGLTexture(m_Context, CL_MEM_READ_ONLY, GL_TEXTURE_2D, 0, scene.GetTexture(0), &error_code);

			if (error_code != CL_SUCCESS)
				return;

			m_Mem[1] = clCreateFromGLTexture(m_Context, CL_MEM_READ_ONLY, GL_TEXTURE_2D, 0, scene.GetTexture(1), &error_code);

			if (error_code != CL_SUCCESS)
				return;

			m_Mem[2] = clCreateFromGLTexture(m_Context, CL_MEM_READ_ONLY, GL_TEXTURE_2D, 0, scene.GetTexture(2), &error_code);

			if (error_code != CL_SUCCESS)
				return;

			m_Mem[3] = clCreateFromGLTexture(m_Context, CL_MEM_READ_ONLY, GL_TEXTURE_2D, 0, scene.GetTexture(3), &error_code);
		...
		void DHOCLTest::Calculate()
		{
			...
			res = clSetKernelArg(m_Kernel, 1, sizeof(m_Mem[1]), &m_Mem[1]);
			res = clSetKernelArg(m_Kernel, 2, sizeof(m_Mem[2]), &m_Mem[2]);
			res = clSetKernelArg(m_Kernel, 3, sizeof(m_Mem[3]), &m_Mem[3]);
			...
			res = clSetKernelArg(m_Kernel, 7, sizeof(m_Mem[4]), &m_Mem[4]);
