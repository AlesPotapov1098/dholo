...
int main()
{
	...
	cl_mem input = clCreateBuffer(context, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR, sizeof(int) * N, global, &err);
	if (err != CL_SUCCESS)
		return -1;

	int result[N];

	cl_mem output = clCreateBuffer(context, CL_MEM_WRITE_ONLY | CL_MEM_USE_HOST_PTR, sizeof(int) * N, result, &err);
	if (err != CL_SUCCESS)
		return -1;
	...
	err = clEnqueueReadBuffer(command_queue, output, CL_TRUE, 0, sizeof(int) * N, result, 0, NULL, NULL);
	if (err != CL_SUCCESS)
		return -1;
